...
def pipeline(source, weights, img_size=640,
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        paint_result(pred, img, im0s, path, dataset, names, colors, img_window, webcam)
...
def paint_result(pred, tensor_img, origin_img, path_img, dataset, names, colors, img_window=False, webcam=False):
    # Process detections
    for i, det in enumerate(pred):  # detections per image
        if webcam:  # batch_size >= 1
            p, s, im0, frame = path_img[i], '%g: ' % i, origin_img[i].copy(), dataset.count
        else:
            p, s, im0, frame = path_img, '', origin_img, getattr(dataset, 'frame', 0)
        p = Path(p)  # to Path
